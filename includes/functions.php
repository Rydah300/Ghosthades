...
function saveExtractionResult($user_id, $keyword, $emails, $targetCount, $batch_id = null, $status = 'completed') {
    ...
    $stmt = $db->prepare("INSERT INTO extractions (user_id, batch_id, keyword, keyword_used, target_count, emails, total, domain_stats, status, processed_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))");
    $stmt->execute([
